...
MeLineFollowArray::MeLineFollowArray(void) : MePort(0)
{

}

MeLineFollowArray::MeLineFollowArray(uint8_t port) : MePort(port)
{

}


MeLineFollowArray::MeLineFollowArray(uint8_t _pin1,uint8_t _pin2)
{
  pin1 = _pin1;
  pin2 = _pin2;
}

void MeLineFollowArray::setpin(uint8_t _pin1,uint8_t _pin2)
{
  pin1 = _pin1;
  pin2 = _pin2;
}
...
bool MeLineFollowArray::readSensor(){
  ...
  //Note : we may skip reading the third byte and save some time
	for(uint8_t k=0; k<3; k++)
	{
			Sensor_Data[k] = 0x00;

      //Read 8 bits encoded with low - high transitions
          ...
          //High bit pulse width 27 uSec=0, 70 uSec=1
				  ...
				  if ( ! (HIGH_level_read_time > 50 && HIGH_level_read_time < 100) )
					{
							Sensor_Data[k] |= (0x80 >> i); // Set bit high when we see a dark line
					}           
			}
	}
